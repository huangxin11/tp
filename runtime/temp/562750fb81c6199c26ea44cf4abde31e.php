<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:64:"D:\www\tp\public/../application/admin/view/default/room\add.html";i:1512042840;s:67:"D:\www\tp\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="main-title">
    <h2>
        <?php echo !empty($room['id'])?'编辑':'新增'; ?>租售信息
    </h2>
</div>
<form action="<?php echo url(); ?>" method="post" class="form-horizontal">
    ...
    <div class="form-item">
        ...
        <div class="controls">
            <input type="text" class="text input-large" name="name" value="<?php echo (isset($room['name']) && ($room['name'] !== '')?$room['name']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">标题<span class="check-tips"></span></label>
        <div class="controls">
            <input type="text" class="text input-large" name="title" value="<?php echo (isset($room['title']) && ($room['title'] !== '')?$room['title']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">价格/万<span class="check-tips"></span></label>
        <div class="controls">
            <input type="text" class="text input-large" name="pricle" value="<?php echo (isset($room['pricle']) && ($room['pricle'] !== '')?$room['pricle']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">电话号码<span class="check-tips"></span></label>
        <div class="controls">
            <input type="text" class="text input-large" name="tel" value="<?php echo (isset($room['tel']) && ($room['tel'] !== '')?$room['tel']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">开始时间<span class="check-tips"></span></label>
        <div class="controls">
            <input type="text" class="text time" name="start_time" value="<?php echo (isset($room['start_time']) && ($room['start_time'] !== '')?$room['start_time']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">结束时间<span class="check-tips"></span></label>
        <div class="controls">
            <input type="text" class="text time" name="end_time" value="<?php echo (isset($room['end_time']) && ($room['end_time'] !== '')?$room['end_time']:''); ?>">
        </div>
    </div>
    <div class="form-item">
        <label class="item-label">是否发布<span class="check-tips"></span></label>
        <div class="controls">
            <input type="radio"  name="status" value="1" <?php echo !empty($room['status']) && $room['status']==1?'checked':''; ?>>是
            <input type="radio"  name="status" value="2" <?php echo !empty($room['status']) && $room['status']==2?'checked':''; ?>>否
        </div>
    </div>
    <div class="form-group">
        <label>类型:</label>
        <select class="item-label" style="width: 200px;" name="room">
            <option value="1" <?php echo !empty($room['room']) && $room['room']==1?'checked':''; ?>>出租</option>
            <option value="2" <?php echo !empty($room['room']) && $room['room']==2?'checked':''; ?>>出售</option>
        </select>
    </div>
    <div class="form-item">
        <label class="item-label">描述<span class="check-tips"></span></label>
        <div class="controls">
            <textarea name="description" style="width: 300px;height: 200px"><?php echo (isset($room['description']) && ($room['description'] !== '')?$room['description']:''); ?></textarea>
        </div>
        <!--<div class="controls">-->
        <!--<select name="target">-->
    ...
    <div class="form-item">
        ...
        <label class="textarea">
            <textarea name="content"><?php echo (isset($room['content']) && ($room['content'] !== '')?$room['content']:''); ?></textarea>
