<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:66:"D:\www\tp\public/../application/admin/view/default/room\index.html";i:1512036435;s:67:"D:\www\tp\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="data-table table-striped">
    <table>
        ...
        <tbody>
        <?php if(!(empty($room) || (($room instanceof \think\Collection || $room instanceof \think\Paginator ) && $room->isEmpty()))): if(is_array($room) || $room instanceof \think\Collection || $room instanceof \think\Paginator): $i = 0; $__LIST__ = $room;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$room): $mod = ($i % 2 );++$i;?>
        <tr>
            <td><input class="ids row-selected" type="checkbox" name="" id="" value="<?php echo $room['id']; ?>"> </td>
            <td><a href="<?php echo url('index?pid='.$room['id']); ?>"><?php echo $room['id']; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo $room['name']; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo $room['description']; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo $room['tel']; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo $room['pricle']; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo !empty($room['room']) && $room['room']==1?'出租':'出售'; ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo time_format($room['start_time']); ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo time_format($room['end_time']); ?></a></td>
            <td><a href="<?php echo url('index?pid='.$servers['id']); ?>"><?php echo !empty($room['status']) && $room['status']==1?'已发布':'未发布'; ?></a></td>
            <td>
                <a title="编辑" href="<?php echo url('edit?id='.$room['id']); ?>">编辑</a>
                <a class="confirm ajax-get" title="删除" href="<?php echo url('shop/del?id='.$shop['id']); ?>">删除</a>
            </td>
        </tr>
